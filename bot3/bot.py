...
async def checkaudio(message:types.Message):
    ...
    # Print out an image with excercise  here
...
async def readingexc(message:types.Message):
    reading = types.ReplyKeyboardMarkup()
    reading.add(types.KeyboardButton(text="I do"))
    await message.reply("Really not so bad. \n\nNext thing we will do is reading. In this task you will see the full text of the story and answer the questions to it. Do you want to try? \n\nJust say “I do”")

@dp.message_handler(Text(equals="I do"))
async def checkreading(message:types.Message):
    readingcheck = types.ReplyKeyboardMarkup()
    readingcheck.add(types.KeyboardButton(text="Ready!"))
    await message.reply(task5, reply_markup=readingcheck)

@dp.message_handler(Text(equals="Ready!"))
async def readq(message:types.Message):
    task = types.ReplyKeyboardMarkup()
    task.add("SOSI HUI")
    await message.reply(task5q, reply_markup=task)

@dp.message_handler(Text(equals="SOSI HUI"))
async def communication(message:types.Message):
    com = types.ReplyKeyboardMarkup()
    com.add("Ready.")
    await message.reply(task6)

@dp.message_handler(Text(equals="Ready."))
async def communicationq(message:types.Message):
    comq = types.ReplyKeyboardMarkup()
    comq.add("I'm ready")
    await message.reply(task6q)

@dp.message_handler(Text(equals="I'm ready"))
async def reflection(message:types.Message):
    reflection = types.ReplyKeyboardMarkup()
    reflection.add("done")
    await message.reply(refl, reply_markup=reflection)

@dp.message_handler(Text(equals="done"))
async def homework(message:types.Message):
    await message.reply("Спасибо за прохождение альфа версии нашего курса также вы можете выполнить домашнее задание в прикрипленном файле")
    # Приложить текстовый файл
